...
`granite-chain-rollup-scope` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for rollup event replay, using fixture event logs and golden state snapshots.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 5.
